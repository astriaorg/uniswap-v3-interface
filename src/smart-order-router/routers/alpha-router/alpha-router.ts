...
import { MethodParameters, Pool, Position, SqrtPriceMath, TickMath } from '@uniswap/v3-sdk'
...
export class AlphaRouter implements IRouter<AlphaRouterConfig>, ISwapToRatio<AlphaRouterConfig, SwapAndAddConfig> {
  ...
  public async route(
    ...
    if (
      ...
      if (protocolsSet.has(Protocol.MIXED) && this.chainId === ChainId.MAINNET && tradeType == TradeType.EXACT_INPUT) {
        log.info({ protocols, swapType: tradeType }, 'Routing across MixedRoutes')
        quotePromises.push(
          this.getMixedRouteQuotes(
            tokenIn,
