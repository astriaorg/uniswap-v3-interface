...
export class FallbackTenderlySimulator implements ISimulator {
  ...
  private async ethEstimateGas(fromAddress: string, route: SwapRoute): Promise<SwapRoute> {
    const currencyIn = route.trade.inputAmount.currency
    const router = SwapRouter02__factory.connect(SWAP_ROUTER_ADDRESS, this.provider)
    const estimatedGasUsed: BigNumber = await router.estimateGas['multicall(bytes[])'](
  ...
  public async simulateTransaction(fromAddress: string, swapRoute: SwapRoute): Promise<SwapRoute> {
...
export class TenderlySimulator implements ISimulator {
  ...
  public async simulateTransaction(fromAddress: string, swapRoute: SwapRoute): Promise<SwapRoute> {
    ...
    if ([ChainId.FLAME_DEVNET].includes(chainId)) {
    ...
    const estimatedGasUsed = BigNumber.from(
      (resp.simulation_results[1].transaction.gas_used * ESTIMATE_MULTIPLIER).toFixed(0)
    )
